...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    await fetch(API + "/todos",{
      method: "POST",
      body: JSON.stringify(todo), // manda o objeto como string
      headers: {
  ...
  setTodos((prevState) => prevState.map((t) => (t.id === data.id) ? (t = data) : t)) //Faz o comparativo com o todo que veio da api
  }
  ...
  return (
    <div className="App">
      ...
      <div className="list-todo">
        ...
        {todos.map((todo) => (
          <div className="todo" key={todo.id}><h3 className={todo.done ? "todo-done": ""}>{todo.title}</h3>
